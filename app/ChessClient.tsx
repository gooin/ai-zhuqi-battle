...
import { drawChessBoard, CHESS_CANVAS_SIZE, type ChessBoardMove } from "../lib/chess/boardCanvas";
import {
  createInitialConversations,
  sideToConversationKey,
} from "../lib/chess/conversation";
...
export default function ChessClient() {
  ...
  useEffect(() => {
    ...
    drawChessBoard(context, board, lastMove);
  }, [board, lastMove]);
  ...
  const statusText = useMemo(() => {
    ...
    if (!started) {
      if (!effectiveConfig.white.ready || !effectiveConfig.black.ready) {
        return "请先配置白黑双方 API URL / Model / API Key，然后点击开始对战";
      }
      return "已就绪，点击“开始对战”";
    }
  ...
  return (
    <main className="app">
      ...
      <section className="centerPanel chessCenterPanel">
        <h1>国际象棋 · 双 LLM 对战</h1>
        <p className="sub">白方先手，白黑双方均由 LLM 控制</p>

        <div className="chessMetaBar">
        ...
        <div className="boardWrap">
          <canvas
            ref={canvasRef}
            width={CHESS_CANVAS_SIZE}
            height={CHESS_CANVAS_SIZE}
            className="board boardChess"
            aria-label="国际象棋棋盘"
          />
        </div>
        ...
        <details className="summary chessTrail">
